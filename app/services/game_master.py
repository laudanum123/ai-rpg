...
class GameMaster:
    ...
    def get_session_memory_graph(self, session_id: str) -> MemoryGraph:
        ...
        if session_id not in self.memory_graphs:
            ...
            try:
                memory_graph.load()
                print(f"Loaded memory graph for session {session_id} with {len(memory_graph.nodes)} nodes")
            except Exception as e:
                print(f"No existing memory graph found for session {session_id} or error loading: {e}")
    ...
    def get_memory_nodes(self, session_id: str, sort_by: str = 'timestamp', reverse: bool = True) -> List[Dict[str, Any]]:
    ...
    def get_node_relations(self, session_id: str, node_id: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def get_ai_response(self, messages: List[Dict], session_id: str = None, recent_history_turns: int = 5) -> str:
            ...
            if history and len(history) > 0:
                ...
                for msg in history:
                    ...
                    if role in ["player", "gm"] and content:
                        ...
                        if not (
                            "inventory" in content.lower() or 
                            "stats:" in content.lower() or
                ...
                for msg in recent_turns:
                    ...
                    openai_role = "user" if role == "player" else "assistant" if role == "gm" else "system"
    ...
    def start_game(self, character: Character, game_world: str, session_id: str = None) -> str:
        ...
        response = self.get_ai_response(messages, session_id=session_id, recent_history_turns=0)
    ...
    def process_action(self, session: 'GameSession', character: Character, action: str, 
                   npcs_present: List[Dict] = None) -> str:
        ...
        movement_words = ["go", "walk", "move", "travel", "head", "enter", "exit", "leave", 
                         "north", "south", "east", "west", "up", "down"]
        potential_movement = any(word in action.lower().split() for word in movement_words)
        ...
        try:
            ...
            if response.choices[0].message.function_call:
                ...
                if "location" in function_args and isinstance(function_args["location"], dict):
                ...
                if "inventory_changes" in function_args and isinstance(function_args["inventory_changes"], dict):
                    inventory_changes = function_args["inventory_changes"]
                    
                    # Process added items
                    if "added_items" in inventory_changes and isinstance(inventory_changes["added_items"], list):
                        for item in inventory_changes["added_items"]:
                            if isinstance(item, dict) and "name" in item and "type" in item:
                                ...
                                new_item = {
                                    ...
                                    "name": item["name"],
                                    "type": item["type"],
                                    "description": item.get("description", f"A {item['type']}.")
                                }
                    ...
                    if "removed_items" in inventory_changes and isinstance(inventory_changes["removed_items"], list):
                    ...
                    if "items_used" in inventory_changes and isinstance(inventory_changes["items_used"], list):
                        for item_name in inventory_changes["items_used"]:
                            # Verify item exists in inventory
                            item_exists = any(inv_item.get("name") == item_name for inv_item in character.inventory)
                ...
                if "character_updates" in function_args and isinstance(function_args["character_updates"], dict):
                ...
                try:
                    ...
                    if "location" in parsed_response and parsed_response["location"].get("location_changed", False):
                        session.current_location = {
                    ...
                    if "inventory_changes" in parsed_response and isinstance(parsed_response["inventory_changes"], dict):
                    ...
                    if "character_updates" in parsed_response and isinstance(parsed_response["character_updates"], dict):
