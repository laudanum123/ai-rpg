...
def new_character():
    ...
    if request.method == "POST":
        ...
        intro_text = game_master.start_game(
            character, game_world, session_id=session_id
        )
...
def game():
    ...
    if not intro_generated:
        intro_text = game_master.start_game(
            character, game_session.game_world, session_id=session_id
        )
        game_state_service.add_message_to_history(session_id, "gm", intro_text)
        game_state_service.set_session_attribute(session_id, "intro_generated", True)
...
def end_combat():
    ...
    return jsonify({"message": "Combat has ended.", "in_combat": False})
...
def debug():
    ...
    if request.method == "POST":
        if "toggle_debug" in request.form:
            ...
            # Set debug mode only on GameMaster (it will propagate to AIService)
            ...
            # Clear logs (only need to clear GameMaster logs now)
    ...
    # Get logs from GameMaster (single source of truth)
    api_logs = []
    if hasattr(game_master, 'api_debug_logs'):
...
def delete_memory_node():
    ...
    if success:
        flash("Memory node deleted successfully", "success")
    else:
        flash("Failed to delete memory node", "danger")

    # Redirect back to memory debug with the same session and view mode
    return redirect(
        url_for("main.memory_debug", session_id=session_id, view_mode="browse")
    )
...
def memory_debug():
    ...
    if request.method == "POST":
        ...
        if query and memory_graph:
            ...
            for node_id, _ in ranked_nodes:
                ...
                node_copy["type_color"] = node_type_colors.get(
                    node.metadata["type"], "secondary"
                )
    ...
    if memory_graph:
        ...
        relation_count = sum(
            len(rel_list) for rel_list in memory_graph.relations.values()
        )
    ...
    if view_mode == "browse" and memory_graph:
        memory_nodes = game_master.get_memory_nodes(
            selected_session_id, sort_by=sort_by, reverse=(sort_order == "desc")
        )
        ...
        if selected_node_id:
            ...
            if selected_node:
                node_relations = game_master.get_node_relations(
                    selected_session_id, selected_node_id
                )
